...
import { deleteKategoriArtikel, saveKategoriArtikel, updateKategoriArtikel } from '@/services/master-data';

const KategoriArtikel = ({data}:{data:any}) => {
    ...
    const save = async () => {
        ...
        const saveing = await saveKategoriArtikel({
            namaKategori: judul
        })
        if(saveing.status) {
            notifications.show({
    ...
    const confirmUpdate = async () =>{
        setLoading(true);
        const update = await updateKategoriArtikel({
            id: parseInt(id),
            namaKategori: judul
        });
        if(update.status) {
    ...
    const confirmDelete = async () => {
        const del = await deleteKategoriArtikel({
            id: parseInt(id)
        });
        if(del.status) {
            notifications.show({
