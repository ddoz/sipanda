...
import { deleteKategoriPangan, deletePasar, saveKategoriPangan, savePasar, updateKategoriPangan, updatePasar } from '@/services/master-data';

const Pasar = ({data}:{data:any}) => {
    ...
    const confirmUpdate = async () =>{
        setLoading(true);
        const update = await updatePasar({
            id: parseInt(id),
            nama: judul
        });
        if(update.status) {
                        ...
                        data.map((item:any, index:number) => (
                            <div key={index} className='w-[250px] px-2 py-1 overflow-hidden border border-gray-300 rounded-md bg-gray-50'>
                                <div className='text-center'>
                                    <span className='text-sm text-slate-600'>Pasar</span>
