...
import { CheckCircleIcon, CheckSquare2Icon, FileIcon, PlusCircle, PrinterIcon, SaveAllIcon, SaveIcon } from 'lucide-react'
...
import { IconChevronLeft, IconChevronRight, IconFile, IconFileExcel, IconFilter, IconInfoCircle, IconPencil, IconSend, IconTrashFilled } from '@tabler/icons-react'
        ...
        <CardContent>
          <div className='flex flex-row gap-2 items-center justify-between w-full'>
            <div className='w-full flex flex-row gap-2 items-center'>
              ...
              <select
                ...
                className="p-1.5 bg-white border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                {
                  panganList.map((item) => (
              ...
              <ActionIcon variant="default" aria-label="show" onClick={handleFilter} size={'lg'}>
                  <IconFilter style={{ width: '70%', height: '70%' }} stroke={1.5} color='red' />
              </ActionIcon>
            </div>
            <div className='w-full items-end justify-end flex flex-row'>
              <Button leftSection={<PrinterIcon />} variant='filled' className='bg-blue-500' onClick={()=>setOpened(true)} >
                Cetak Laporan
              </Button>
            </div>
          </div>
          ...
          >
            <div className='flex flex-col gap-3'>
              ...
              <Button loading={loadingExport} leftSection={<IconFileExcel />} variant='filled' className='bg-blue-500' onClick={()=>exportExcel()} >
                Export
              </Button>
            </div>
          </Modal>
                          ...
                          return (
                              <TableCell key={dateIndex}>
                                  <div className='flex flex-col items-center gap-1'>
                                      {/* {(todayDate == dateKey) &&  */}
                                      <TextInput placeholder='Input Harga' onChange={(e) => handleInputChange(panganItem.id, pasarItem.id, dateKey, e.target.value)} />
                                      {/* } */}
