...
const Slider = ({data}:{data:any}) => {
    ...
    const save = async () => {
        ...
        const saveing = await saveSliders({
            title: judul,
            file: file
        })
        if(saveing.status) {
    ...
    const deleteSlider = (id:any) => {
        setOpenDialog(!openDialog)
        setIdGaleri(id);
    }

    const confirmDelete = async () => {
        const del = await deleteSliders({
                        ...
                        data.map((item:any, index:number) => (
                            <div key={index} className='w-[200px] h-[200px] overflow-hidden border border-gray-300 rounded-md relative'>
                                <div  className='absolute top-1 right-1'>
                                    <ActionIcon variant="transparent" aria-label="delete" onClick={()=>deleteSlider(item.id)}>
                                        <IconTrashFilled style={{ width: '70%', height: '70%' }} stroke={1.5} color='red' />
                                    </ActionIcon>
                                </div>
                               <ViewFileComponent filePath={item.image} title='' />
